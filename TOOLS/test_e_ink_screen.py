...
def main():
    ...
    if not opts.secondary_io_pin:
        test_ios = opts.primary_io_pin
    else:
        test_ios = [opts.primary_io_pin, opts.secondary_io_pin]
    ...
    while True:
        ...
        if not opts.secondary_io_pin:
            io_extender.write_io(test_ios, 1)
        else:
            io_extender.write_ios(test_ios, 1)

        time.sleep(opts.high_level_duration / 1000.0)

        log(DEBUG, 'Test toggling IO(s) low')

        if not opts.secondary_io_pin:
            io_extender.write_io(test_ios, 0)
        else:
            io_extender.write_ios(test_ios, 0)

        time.sleep(opts.low_level_duration / 1000.0)
